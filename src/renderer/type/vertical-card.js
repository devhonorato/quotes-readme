...
/**
 * Renderiza um SVG para um card de citação vertical.
 *
 ...
 */
const renderVertical = ({ quote, author, id, color, border, width, height }) => {
  const padding = 20; // Espaçamento interno do card
  const borderSize = border ? 3 : 1; // Tamanho da borda, se definido
  const maxWidth = width || 350; // Largura máxima do card, padrão de 350 se não fornecido
  ...
  const totalHeight = quoteMeasurements.height + authorMeasurements.height + padding * 2 + 50;
        ...
        <div xmlns="http://www.w3.org/1999/xhtml">
          ...
          <style>
            * {
            ...
            .container {
              background-color: #${themes.default.background}; /* Cor de fundo do card */
            }
            .container h3 {
              color: #${themes.default.quote}; /* Cor do texto da citação */
            }
            .container h3::before, .container h3::after {
              color: #${themes.default.symbol}; /* Cor dos símbolos de citação */
            }
            .container p {
              color: #${themes.default.author}; /* Cor do texto do autor */
            }
            @media (prefers-color-scheme: dark) {
              .container {
                background-color: #${themes.defaultDarkModeSupport.background}; /* Cor de fundo para o modo escuro */
                border: ${border ? "3px solid #" + themes.defaultDarkModeSupport.symbol : "1px solid rgba(0, 0, 0, 0.2)"};
              }
              .container h3 {
                color: #${themes.defaultDarkModeSupport.quote}; /* Cor do texto da citação para o modo escuro */
              }
              .container h3::before, .container h3::after {
                color: #${themes.defaultDarkModeSupport.symbol}; /* Cor dos símbolos de citação para o modo escuro */
              }
              .container p {
                color: #${themes.defaultDarkModeSupport.author}; /* Cor do texto do autor para o modo escuro */
              }
            }
            ${color !== themes.default && color !== themes.defaultDarkModeSupport ? 
            `.container {
                background-color: #${color.background}; /* Cor de fundo personalizada */
                border: ${border ? "3px solid #" + color.symbol : "1px solid rgba(0, 0, 0, 0.2)"};
              }
              .container h3 {
                color: #${color.quote}; /* Cor do texto da citação personalizada */
              }
              .container h3::before, .container h3::after {
                color: #${color.symbol}; /* Cor dos símbolos de citação personalizados */
              }
              .container p {
                color: #${color.author}; /* Cor do texto do autor personalizada */
              }
            ` : ''}
          </style>
...
module.exports = {
  renderVertical // Exporta a função renderVertical para uso em outros módulos
};
